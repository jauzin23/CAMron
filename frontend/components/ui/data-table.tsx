    ...
    <div className="space-y-4">
      {(filterColumnKey || actionButton) && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          {actionButton && (
            <div className="w-full sm:w-auto sm:ml-auto">{actionButton}</div>
          )}
        </div>
      )}
        ...
        <Table className="w-full">
          <TableHeader className="bg-zinc-950 border-b border-zinc-800">
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow
                key={headerGroup.id}
                className="hover:bg-transparent border-b border-zinc-800"
              >
                {headerGroup.headers.map((header) => {
                  const isIpOrCreatedAt =
                    header.id === "ip" || header.id === "created_at";
